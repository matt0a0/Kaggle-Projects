...
class Modelo1(nn.Module):
    def __init__(self, in_features=1, num_classes=10, dim=1024):
        ...
        self.fc1 = nn.Linear(64*5*5, dim)  # Ajustando a dimensão da primeira camada totalmente conectada
        self.fc = nn.Linear(dim, num_classes)  # Ajustando o nome para fc para nãp dar erro
    ...
    def forward(self, x):
        ...
        out = nn.functional.softmax(self.fc(out), dim=1)  # Usando a camada renomeada fc
        return out
#Resnet
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # Se os canais de entrada e de saída forem diferentes, precisamos adicionar um bloco de identidade
    ...
    def forward(self, x):
        ...
        # Se o downsample existir, aplicamos no identity
...
class Modelo2(nn.Module):
    def __init__(self, in_channels=1, num_classes=10):
        ...
        self.layer1 = self.make_layer(64, 64, 3, stride=1)  # Ajustado o número de canais de entrada
        self.layer2 = self.make_layer(64, 128, 4, stride=2)  # Ajustado o número de canais de entrada
        self.layer3 = self.make_layer(128, 256, 6, stride=2)  # Ajustado o número de canais de entrada
        self.layer4 = self.make_layer(256, 512, 3, stride=2)  # Ajustado o número de canais de entrada
    ...
    def make_layer(self, in_channels, out_channels, blocks, stride=1):
        layers = []
        layers.append(ResidualBlock(in_channels, out_channels, stride))  # Ajustado o número de canais de entrada
...
class Modelo3(nn.Module):
    def __init__(self, in_features=1, num_classes=10):
        ...
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(28 * 28 * in_features, 128)
        self.relu = nn.ReLU(inplace=True)
...
#Classe que realiza o ensemble das redes 
class MyEnsemble(nn.Module):
    def __init__(self, num_classes=10, dim=1024):
        super(MyEnsemble, self).__init__()
        self.model1 = Modelo1(in_features=1, num_classes=num_classes, dim=dim) 
        self.model2 = Modelo2(num_classes=num_classes)
        self.model3 = Modelo3(in_features=1, num_classes=num_classes)
        self.fc = nn.Linear(3 * num_classes, num_classes)  # Adicionando a camada totalmente conectada para o ensemble

    def forward(self, x):
        out1 = self.model1(x)
        out2 = self.model2(x)
        out3 = self.model3(x)
        # Concatenando as saídas dos modelos
        out = torch.cat((out1, out2, out3), dim=1)
        # Aplicando a camada totalmente conectada para o ensemble
        out = nn.functional.softmax(self.fc(out), dim=1)
        return out